...
#'
#' @param GTs Numeric matrix of genotype dosages, individuals x markers, with
#'   column names matching `map$marker`.
...
#' @param ld_w_col,ld_w_threshold Optional. If both are supplied, markers
#'   with `map[[ld_w_col]] < ld_w_threshold` skip clustering entirely and
#'   pass straight through as their own singleton cluster -- useful when
#'   most markers are already known to carry little local LD support and
#'   clustering all of them anyway would be wasted work.
#'
#' @return A list with:
#' \describe{
#'   \item{map_snp}{`map`, with `CL_id`, `n_loci`, `is_core` (logical), and
#'     `median_ld` (this marker's own median r2 to the rest of its cluster,
#'     `NA` for singletons) added. `CL_id`/`n_loci` are named to match what
#'     [make_eMLGs()] expects from `map_cl`.}
...
#'
#' @export
ld_complexity_reduction <- function(GTs, map, LD_decay, rho = 0.5, cores = 1,
  ...
  by_chr <- parallel_apply(chr_levels, function(ch) {
    ...
    if (!is.null(ld_w_col) && !is.null(ld_w_threshold)) {
      independent_markers <- chr_map[get(ld_w_col) < ld_w_threshold, marker]
      chr_map <- chr_map[get(ld_w_col) >= ld_w_threshold]
    } else {
      independent_markers <- character(0)
    }
    ...
    if (length(chr_markers) < 2) {
      out <- rbind(singletons(chr_markers), singletons(independent_markers))
      out[, Chr := ch]
      return(out)
    }
    ...
    per_component <- lapply(split(names(comp$membership), comp$membership), function(cluster_markers) {
    ...
    out <- rbind(data.table::rbindlist(per_component), singletons(independent_markers))
    out[, Chr := ch]
    out
  }, cores = cores)
  ...
  map_snp <- merge(
    data.table::copy(map),
    long[, .(marker, CL_id, n_loci, is_core, median_ld)],
