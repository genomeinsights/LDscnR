...
**Why complete linkage on real pairwise values?** Most LD-pruning tools (e.g. PLINK's `--indep-pairwise`) make a single greedy pass along a sliding window: within each window, if a pair exceeds the r² threshold, one member is dropped, and the window moves on. That only ever checks pairs that happen to co-occur in the same window at the same step -- it never verifies that the *whole surviving set* is mutually independent. Two "kept" markers can still be correlated with each other if the marker that would have bridged that comparison was the one removed earlier for an unrelated pair. That matters because the entire point of pruning is to end up with genuinely independent test units: residual correlation among "kept" markers silently inflates the effective marker count and biases FDR correction, GRM/kinship estimation, or association testing, without showing up anywhere in the pruned set itself.

Stage 1 avoids this by construction: within each connected component, `hclust()` clusters on the real pairwise r² sub-matrix using complete linkage, so a marker only joins a cluster if it clears the threshold against *every* existing member, not just its nearest neighbour. That directly rules out single linkage's "chaining" failure (A uncorrelated with C, but both pulled into one group via an intermediate B correlated with each) -- the same failure mode that undermines greedy sequential pruning, since a pair is never re-checked once one of its members has already been removed for an unrelated reason. The representative marker for each cluster is chosen the same principled way: highest median r² to the rest of its own cluster, not an arbitrary or greedy pick. The result is markers in *different* final clusters are, by construction, never verified as mutually redundant with each other -- about as close to "each retained marker is a genuinely independent test unit" as a threshold-based method gets.
